...
void ShiftControlMethod::Init(RobotSpec spec)
{
    ...
    /*  初期値0をデックに追加しておく   */
   	for(int i=0; i<NUM_JOINT; i++){
   		...
   		//hold_data.shift_param[i].phi_hold.resize(max_hold_num, 0);
   	}
    ...
    //snake_model_param.phi.resize(NUM_JOINT, 0);
}
...
void ShiftControlMethod::Shift_Param(RobotSpec spec)
{
	...
	//hold_data.shift_param[0].phi_hold.insert(hold_data.shift_param[0].phi_hold.begin(), phi_);
	...
	for(int i=0; i<NUMOFLINK; i++){
        ...
        //snake_model_param.phi.insert(snake_model_param.phi.begin(), hold_data.shift_param[i].phi_hold[hold_num-1]);
